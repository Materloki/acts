...
///MultiTrackNavigator"
enum class MultiTrackNavigatorBranchStopperResult {
  Continue,
  StopAndDrop,
  StopAndKeep,
}; 
...
struct MultiTrackNavigatorExtensions {
  ...
  using BranchStopperResult = MultiTrackNavigatorBranchStopperResult;
...
static constexpr std::size_t size_maxBranchesPerSurface = 10;

namespace MtnTypes {

template <typename T>
using BranchVector = boost::container::small_vector<T, size_maxBranchesPerSurface>;

}  // namespace MtnTypes

/// Combined options for the MTN.
///
...
struct MultiTrackNavigatorOptions {
  ...
  MultiTrackNavigatorOptions(
      const GeometryContext& gctx, const MagneticFieldContext& mctx,
      std::reference_wrapper<const CalibrationContext> cctx,
      SourceLinkAccessor accessor_,
      MultiTrackNavigatorExtensions<track_container_t> extensions_,
  ...
  MultiTrackNavigatorOptions() = delete;
  ...
  MultiTrackNavigatorExtensions<track_container_t> extensions;
  ...
  using TrackStateCandidateCreator =
      Delegate<Result<MtnTypes::BranchVector<TrackIndexType>>(
...
template <typename track_container_t>
struct MultiTrackNavigatorResult {
  using TrackStateContainerBackend =
      typename track_container_t::TrackStateContainerBackend;
  using TrackProxy = typename track_container_t::TrackProxy;
  using TrackStateProxy = typename track_container_t::TrackStateProxy;
...
/// MultiTrackNavigator to find tracks.
///
/// @tparam propagator_t Type of the propagator
///
/// The MultiTrackNavigator contains an Actor and a Sequencer sub-class.
...
/// MultiTrackNavigator, measurement ordering needs to be figured out by
/// the navigation of the propagator.
///
/// The void components are provided mainly for unit testing.
///
template <typename propagator_t, typename track_container_t>
class MultiTrackNavigator {
 public:
  /// Default constructor is deleted
  MultiTrackNavigator() = delete;
  /// Constructor from arguments
  MultiTrackNavigator(propagator_t pPropagator,
                            std::unique_ptr<const Logger> _logger =
                                getDefaultLogger("MTN", Logging::INFO))
 ...
 private:
  ...
  struct DefaultTrackStateCreator {
     typename MultiTrackNavigatorExtensions<track_container_t>::Calibrator
        calibrator;
    typename MultiTrackNavigatorExtensions<
    ...
    template <typename source_link_iterator_t>
    Result<MtnTypes::BranchVector<TrackIndexType>> createSourceLinkTrackStates(
      ...
      using ResultTrackStateList =
          Acts::Result<MtnTypes::BranchVector<TrackIndexType>>;
      ResultTrackStateList resultTrackStateList{
          MtnTypes::BranchVector<TrackIndexType>()};
    ...
    /// @param logger the logger for messages
    Result<MtnTypes::BranchVector<TrackIndexType>> processSelectedTrackStates(
      ...
      ResultTrackStateList resultTrackStateList{
          MtnTypes::BranchVector<TrackIndexType>()};
      MtnTypes::BranchVector<TrackIndexType>& trackStateList =
          *resultTrackStateList;
  ...
  class Actor {
   public:
    ...
    using result_type = MultiTrackNavigatorResult<track_container_t>;
    ...
    void operator()(propagator_state_t& state, const stepper_t& stepper,
      ...
      ACTS_VERBOSE("MultiTrackNavigator step");
    ...
    /// @brief MultiTrackNavigator actor operation:
    ...
    Result<void> filter(const Surface* surface, propagator_state_t& state,
      ...
      if (auto [slBegin, slEnd] = m_sourcelinkAccessor(*surface);
        ...
        const MtnTypes::BranchVector<TrackIndexType>& newTrackStateList =
            *tsRes;
        ...
        if (nStoppedBranches >= newTrackStateList.size()) {
          ...
          if (currentBranch.outermostTrackState().typeFlags().test(
            ...
            stepper.update(state.stepping,
                           transformBoundToFreeParameters(ts.referenceSurface(),
    ...
    Result<std::pair<unsigned int, unsigned int>> processNewTrackStates(
      ...
      MtnTypes::BranchVector<TrackProxy> newBranches;
      ...
      for (TrackProxy newBranch : newBranches) {
        ...
        if (typeFlags.test(TrackStateFlag::OutlierFlag)) {
          ...
          // Even if we dont do the Kalman update is necessary to have
          // filtered estimates  
    ...
    MultiTrackNavigatorExtensions<track_container_t> m_extensions;
    ...
    using TrackStateCandidateCreator =
        typename MultiTrackNavigatorOptions<
  ...
  /// @param tfOptions MultiTrackNavigatorOptions steering the track
  ///                  finding
  ...
  auto findTracks(const start_parameters_t& initialParameters,
    ...
    using MultiTrackNavigatorAborter =
        Aborter<SourceLinkAccessor, parameters_t>;
    using MultiTrackNavigatorActor =
        Actor<SourceLinkAccessor, parameters_t>;
    using Actors = ActionList<MultiTrackNavigatorActor>;
    using Aborters = AbortList<MultiTrackNavigatorAborter>;
    ...
    auto& multiTrackActor =
        propOptions.actionList.template get<MultiTrackNavigatorActor>();
    ...
    multiTrackActor.m_sourcelinkAccessor = tfOptions.sourcelinkAccessor;
    multiTrackActor.m_extensions = tfOptions.extensions;
    multiTrackActor.trackStateCandidateCreator =
    ...
    if (!multiTrackActor.trackStateCandidateCreator.connected()) {
      ...
      multiTrackActor.trackStateCandidateCreator.template connect<
    ...
    auto& r =
        propState
            .template get<MultiTrackNavigatorResult<track_container_t>>();
    ...
    auto multiTrackResult =
        std::move(propRes.template get<
                  MultiTrackNavigatorResult<track_container_t>>());
    ...
    if (!error.ok()) {
      ACTS_ERROR("MultiTrackNavigator failed: "
    ...
    for (const auto& track : multiTrackResult.collectedTracks) {
