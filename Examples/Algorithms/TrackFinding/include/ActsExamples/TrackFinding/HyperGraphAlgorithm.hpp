...
class HyperGraphAlgorithm final : public IAlgorithm {
    public:
        /// Track finder function that takes input measurements, initial trackstate
        /// and track finder options and returns some track-finder-specific result.
        using HyperGraphOptions =
            Acts::MultiTrackNavigatorOptions<IndexSourceLinkAccessor::Iterator,
                                                    TrackContainer>;
        using HyperGraphResult =
        ...
        public:
            virtual ~TrackFinderFunction() = default;
            virtual HyperGraphResult operator()(const TrackParameters&,
                                                const HyperGraphOptions&,
                                                TrackContainer&) const = 0;
        };
        ...
        /// Constructor of the hyper graph algorithm
        ///
        /// @param config is the config struct to configure the algorithm
        /// @param level is the logging level
        HyperGraphAlgorithm(Config config, Acts::Logging::Level level);
