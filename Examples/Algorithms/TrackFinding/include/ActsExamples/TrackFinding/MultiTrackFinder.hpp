...
#include "ActsExamples/TrackFinding/MultiTrackFinderExtensions.hpp"
#include <system_error>
#include <type_traits>

namespace Acts {

/// Combined options for the Muli track finder.
///
...
struct MultiTrackFinderOptions {
...
/// @param extensions The extension struct
/// @param pOptions The plain propagator options
/// @param mScattering Whether to include multiple scattering
/// @param eLoss Whether to include energy loss
MultiTrackFinderOptions(
    const GeometryContext& gctx, const MagneticFieldContext& mctx,
    std::reference_wrapper<const CalibrationContext> cctx,
    MultiTrackFinderExtensions<track_container_t> extensions_,
    const Acts::PropagatorPlainOptions& pOptions, bool mScattering = true,
    bool eLoss = true)
    : geoContext(gctx),
  ...
  MultiTrackFinderOptions() = delete;

  /// Context object for the geometry
  std::reference_wrapper<const GeometryContext> geoContext;
  /// Context object for the magneticc field
  ...
  MultiTrackFinderExtensions<track_container_t> extensions;
  /// The trivial propagator options
  Acts::PropagatorPlainOptions propagatorPlainOptions;
  ...
  /// Wheter to consider multiple scattering.
...
class MultiTrackFinder {
  public:
    ...
    explicit MultiTrackFinder(propagator_t pPropagator,
                              std::unique_ptr<const Logger> _logger = 
                                  getDefaultLogger("MTF", Logging::INFO))
    ...
    private:
      ...
      /// @brief Propagator Actor plugin for the MultiTrackFinder
      ///
      /// The MultiTrackFinder Actor does not realy on the measurements to
      ...
      class Actor {
        ...
        using result_type = MultiTrackFinderResult<track_container_t>;
        ...
        /// Wheter to consider energy loss
        bool energyLoss = true;
        ...
        /// @brief MultiTrackFinder actor operation
        ///
        ...
        void act(propagator_state_t& state, const stepper_t& stepper,
          ...
          ACTS_VERBOSE("MultiTrackFinder step");
          ...
          if (auto surface = navigator.currentSurface(state.navigation);
              surface != nullptr) {
            ACTS_VERBOSE("Perform filter? maybe");
        ...
        /// @brief MultiTrackFinder actor operation: reset propagation
        ///
        ...
        void reset(propagator_state_t& state, const stepper_t& stepper,
          ...
          if(result.activeBranches.empty()) {
            ACTS_VERBOSE("Stop MTF with " << result.collectedTracks.size()
                                          << " found tracks");
          ...
          // No Kalman filtering for the starting surface, but still need to consider
          // the material effects here
          /// FIXME: Do we really need to?
          /*
        ...
        /// @param state     The mutable propagator state onject
        ...
        Result<void> filter(const Surface* surface, propagator_state_t& state,
          ...
          using TrackStatesResult = 
              Acts::Result<MtfTypes::BranchVector<TrackIndexType>>;
          TrackStatesResult tsRes = TrackStatesResult::success({});
          if (isSensitive) {
            // extended trajectory with measurements associated to the current surface
          ...
          if (tsRes.ok() && !(*tsRes).empty()) {
            const MtfTypes::BranchVector<TrackIndexType>& newTrackStateList = *tsRes;
            ...
            if (!tsRes.ok()) {
              if (static_cast<MultiTrackFinderError>(
                    tsRes.error().value()) ==
                  MultiTrackFinderError::NoMeasurementExpected) {
                // reciverable error returned by track state creator
            ...
            if(branchStopperResult == BranchStopperResult::Continue) {
              // Remember the active branch and its state
            } else {
        ...
        /// @FIXME: Also should not be necessary
        /*
      ...
      /// Process new, incomplete track states and set the filtered state
      ///
      ...
      /// @param reuslt which contains among others the new states, and the list of active branches
      /// @return the number of newly added branches or an error
      Result<unsigned int> processNewTrackStates(
          const Acts::GeometryContext& gctx,
          const MtfTypes::BranchVector<TrackIndexType>& newTrackStateList,
          result_type& result) const {
        using PM = TrackStatePropMask;
        ...
        MtfTypes::BranchVector<TrackProxy> newBranches;
        ...
        for (TrackProxy newBranch : newBranches) {
          ...
          if(typeFlags.test(TrackStateFlag::OutlierFlag)) {
            ...
            // Set the filtered parameter index to be the same with predicted parameter
            ...
            // Should be Kalman update, but we dont do this on this house
            /* 
            auto updateRes = 
          ...
          if (branchStopperResult == BranchStopperResult::Continue) {
            ...
            // Record the number of stopped branches on surface
            if (branchStopperResult == BranchStopperResult::StopAndKeep) {
              storeLastActiveBranch(result);
            }
      ...
      /// @brief MultiTrackFinder actor operation: add a hole or material track state
      /// 
      ...
      /// @param expectMeasurements True if measurements were expected for this surface
      ...
      TrackIndexType addNonSourcelinkState(TrackStatePropMask stateMask,
        ...
        // Add a track state
        auto trackStateProxy = result.trackStates->makeTrackState(stateMask, prevTip);
        ACTS_VERBOSE("Create "
                      << (isSensitive
        ...
        // Set the filtered parameter index to be the sme with predicted parameter
        trackStateProxy.shareFrom(PM::Predicted, PM::Filtered);

        return trackStateProxy.index();
      }
      
      /// @brief MultiTrackFinder actor operation: material interaction (lets try without it)
      ///
      /// @tparam propagator_state_t is the type of Propagator state
      /// @tparam stepper_t Tupe of the stepper
      /// @tparam navigator_t Type of the navigator
      /// 
      ...
      MultiTrackFinderExtensions<track_container_t> m_extensions;
...
/// limit is handled in the MTF actor internally
...
public:
  /// Multi Track Finder implementation, calls the finder
  ///
  /// @tparam start_parameters_t Type of the initial parameters
  /// @tparam parameters_t Type of parameters used for local parameters
  ///
  /// @param initialParameters The initial track parameters
  /// @param tfOptions MultiTrackFinderOptions steering the track finding
  ...
  ///       It's @c calibrations_t's job to turn them into calibrated measurements
  ///       used in the track finding
  ///
  /// @return a conteiner of a track finding result for all the initial track parameters

  template <typename start_parameters_t>
  auto findTracks(
      const start_parameters_t& initialParameters,
      const MultiTrackFinderOptions<track_container_t>& tfOptions,
    ...
    // Create he ActorList
    using MultiTrackFinderActor = Actor;
    using Actors = ActorList<MultiTrackFinderActor>;
    ...
    // Set the trivial propator options
    propOptions.setPlainOptions(tfOptions.propagatorPlainOptions);

    // Catch the actor
    auto& multiTrackActor = 
        propOptions.actorList.template get <MultiTrackFinderActor>();
    ...
    multiTrackActor.m_extensions = tfOptions.extensions;
    ...
    auto& r = 
        propState
            .template get<MultiTrackFinderResult<track_container_t>>();
    ...
    // Get the result of the MultiTrackFinder
    auto multiTrackResult =
        std::move(propRes.template get<
                  MultiTrackFinderResult<track_container_t>>());
    
    Result<void> error = multiTrackResult.lastError;
    if (error.ok() && !multiTrackResult.finished) {
      error = Result<void>(
          MultiTrackFinderError::PropagationReachesMaxSteps);
    }
    if (!error.ok()) {
      ACTS_ERROR("MultiTrackFinder failed: "
                  << multiTrackResult.lastError.error() << " "
                  << multiTrackResult.lastError.error().message()
                  << " with the initial parameters: "
                  << initialParameters.parameters().transpose());
      return error.error();
    }

    return std::move(multiTrackResult.collectedTracks);
  }

  /// Multi Track Finder implementation, calls the MTF
  ///
  /// @tparam start_parameters_t Type of the initial parameters
  /// @tparam parameters_t Type of parameters used for local parameters
  ///
