...
void visitSeedIdentifiers(const TrackProxy& track, Visitor visitor) {
  // first we collect the osurce link indices of the track states
...
class BranchStopper {
  public:
   using BranchStopperResult = 
      Acts::MultiTrackFinderBranchStopperResult;
...
ProcessCode HyperGraphAlgorithm::execute(const AlgorithmContext& ctx) const {
  ...
  */
  ...
  using Extensions = Acts::MultiTrackFinderExtensions<TrackContainer>;
  ...
  */
  ...
  // Set the MultiTrackFinder options
  ...
  //firstOptions.skipPrePropagationUpdate = true;
...
ProcessCode HyperGraphAlgorithm::finalize() {
  ACTS_INFO("HyperGraphAlgorithm statistics:");
  ...
  ACTS_INFO("- failure raito seeds: " << static_cast<double>(m_nFailedSeeds) /
                                            m_nTotalSeeds);
...
void HyperGraphAlgorithm::computeSharedHits(
