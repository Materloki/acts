...
Acts::Result<std::vector<Acts::Vertex>> Acts::TDVFinder3D::find(
    const std::vector<InputTrack>& trackVector,
    const VertexingOptions& vertexingOptions,
    IVertexFinder::State& /*state*/) const {
  GaussianTrackDensity3D::State densityState(trackVector.size());
  ...
  Vector3 zt = zAndWidth.first;

  // Calculate seed position
  // Note: constraint position is (0,0,0) if no constraint provided
  Vector4 seedPos =
      vertexingOptions.constraint.fullPosition() + Vector4(0., 0., zt(1), zt(2));
